...
const SectionImage = () => {
  ...
  const imageInView = useInView(containerRef, { amount: 0.75, once: true });
  ...
  const variant_bg_full = {
    show: {
      height: imageBiggerInView ? `100%` : `0%`,
      backgroundSize: imageBiggerInView ? "130%" : "100%",
    },
  };
      ...
      <div className="flex h-[50%] gap-5 lg:h-[873px]" ref={containerRef}>
        <motion.div className="basis-1/2 overflow-hidden rounded-[20px]">
          <motion.div
            ...
            transition={{
              duration: 1,
              backgroundSize: { delay: 0.2, duration: 1, ease: "easeInOut" },
            }}
            animate="show"
